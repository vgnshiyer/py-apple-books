...
class BooksApi:
    ...
    def list_collections(self, include_books: bool = False) -> list[Collection]:
        ...
        if not raw_collection_data:
            return []
        list_of_collections = []
        collections = [dict(zip(Collection.__annotations__.keys(), c)) for c in raw_collection_data]
    ...
    def get_collection_by_id(self, collection_id: str, include_books: bool = False) -> Optional[Collection]:
        ...
        collection = dict(zip(Collection.__annotations__.keys(), raw_collection_data))
    ...
    def list_books(self) -> list[Book]:
        ...
        if not raw_book_data:
            return []
        list_of_books = []
        books = [dict(zip(Book.__annotations__.keys(), b)) for b in raw_book_data]
        for book in books:
            book_obj = Book(**book)
    ...
    def get_book_by_id(self, book_id: str) -> Optional[Book]:
        ...
        book = dict(zip(Book.__annotations__.keys(), raw_book_data))
        return Book(**book)
