...
def find_by_id(collection_id: str, books: bool = False) -> list:
    ...
    query = query_utils.get_book_collection_query() + \
        f"WHERE {query_utils.COLLECTION_TABLE_NAME}.Z_PK = {collection_id}"
...
def find_by_name(collection_name: str, books: bool = False) -> list:
    ...
    query = query_utils.get_book_collection_query() + \
        f"WHERE {query_utils.COLLECTION_TABLE_NAME}.ZTITLE = {collection_name}"
    return db_utils.run_query(query)
...
def find_by_book_id(book_id: str) -> list:
    ...
    """
    query = query_utils.get_book_collection_query() + \
        f"WHERE {query_utils.BOOK_TABLE_NAME}.Z_PK = {book_id}"
    return db_utils.run_query(query)
